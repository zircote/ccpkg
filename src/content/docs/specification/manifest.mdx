...
| `checksum` | OPTIONAL | `string` | Format: `"sha256:<hex>"`. 64 hex characters after prefix. | SHA-256 hash computed over the raw archive bytes (the `.ccpkg` ZIP file), excluding the checksum field itself from the manifest before hashing. |
...
| `instructions` | `string` or `object` | Instructions declaration. A string is a path to a single instructions file. An object declares a base file and optional per-host overlays for assembly. |
